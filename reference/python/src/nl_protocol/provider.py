...
if TYPE_CHECKING:
    ...
    from nl_protocol.isolation.subprocess import IsolatedExecutor
...
class NLProvider:
    ...
    * **Level 7** -- Cross-Agent Trust & Federation (delegation tokens)
    ...
    Level 3 (Execution Isolation) is integrated into the pipeline for
    ``EXEC`` actions when enabled.  Commands are executed in an isolated
    subprocess with secret injection via environment variables.
    ...
    def __init__(
        ...
        self._isolated_executor: IsolatedExecutor | None = None
        if 3 in config.supported_levels:
            from nl_protocol.isolation.subprocess import IsolatedExecutor
    ...
    @property
    def isolated_executor(self) -> IsolatedExecutor | None:
        """The Level 3 isolated executor (``None`` if L3 disabled)."""
    ...
    async def process_action(self, request: ActionRequest) -> ActionResponse:
        ...
        try:
            ...
            # -- Step 4: Resolve placeholders + build execution context ----
            resolved_secrets: list[str] = []
            secrets_dict: dict[str, str] = {}
            secret_values: dict[str, SecretValue] = {}
            exec_command = request.action.template

            if decision.secret_refs:
                for i, ref in enumerate(decision.secret_refs):
                    sv = await self._secret_store.get(ref)
                    secrets_dict[str(ref)] = sv.expose()
                    secret_values[str(ref)] = sv
                    # Replace {{nl:ref}} with $NL_SECRET_<i> for env-var injection
                    exec_command = exec_command.replace(
                        f"{{{{nl:{ref}}}}}", f"$NL_SECRET_{i}"
                    )
                resolved_secrets = [str(r) for r in decision.secret_refs]
            ...
            # -- Step 5: Execute action (Level 3) --------------------------
            should_execute = (
                request.action.type == ActionType.EXEC
                and self._isolated_executor is not None
            )

            if should_execute:
                result = await self._isolated_executor.execute(
                    command=exec_command,
                    secrets=secrets_dict or None,
                    timeout=request.action.timeout,
                )
                # -- Step 5b: Sanitize output (Level 2) --------------------
                if secret_values:
                    result = ActionResult(
                        exit_code=result.exit_code,
                        stdout=self._output_sanitizer.sanitize(
                            result.stdout, secret_values
                        )[0],
                        stderr=self._output_sanitizer.sanitize(
            ...
            # -- Step 6: Audit recording (Level 5 -- SHA-256 chain) ---------
